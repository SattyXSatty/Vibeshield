...
### 📂 Data Directory
* **Root Path**: `/Users/satyendrasahani/Documents/EAG2/S15_NewArch/data`
* **Rule**: All data files (datasets, inputs, outputs) MUST be read from or saved to this directory. Do not assume any other storage location.

### 🌐 Browser Usage
* **NO LOCAL BROWSER**: Logic like `selenium`, `playwright` (local launch), or `webdriver` DOES NOT EXIST.
* **MUST USE MCP**: You must assign web-related tasks to **BrowserAgent** (or agents using `server_browser` tools).
* **Strict Rule**: Never plan a step that assumes appropriate browser binaries are installed locally. Always delegate to the internal tool ecosystem.

### 🕒 Search Freshness
* **Tool Capability**: The `web_search` tool (used by RetrieverAgent) supports a `freshness` parameter: `'d'` (day), `'w'` (week), `'m'` (month), `'y'` (year).
* **Strategy**: For queries involving "latest", "recent", "today", or current events (e.g. 2025/2026 data), explicitly instruct the `RetrieverAgent` in its `agent_prompt` to use an appropriate freshness filter (usually `'m'` or `'w'`).

### 📚 RAG-First Strategy
* **Tool Capability**: The `search_stored_documents_rag` tool (used by RetrieverAgent) searches internal documents that have been indexed.
* **Critical Rule**: For queries about **personal data, internal records, or specific named entities** (e.g., "Anmol Singh's apartment", "our Q3 report", "John's contract"), **ALWAYS** instruct the `RetrieverAgent` to use `search_stored_documents_rag` **FIRST** before attempting web searches.
* **Pattern Recognition**: Queries containing proper names, possessive pronouns ("my", "our"), or references to internal documents should trigger RAG searches.
* **Fallback**: Only use `web_search` if the query is clearly about public/external information (e.g., "latest news", "Wikipedia definition", "current stock price").
...
* `file_manifest`: Metadata list of available files, including:
  - Files from the `data/` directory (marked with `source: "data"`)
  - **RAG-indexed documents** from `mcp_servers/documents/` (marked with `source: "rag_indexed"`)
  - Each entry includes: `filename`, `path`, `size`, `type`, `source`

**CRITICAL**: If `file_manifest` contains documents with `source: "rag_indexed"` that are relevant to the query (e.g., "How to use Canvas LMS.pdf" for a Canvas LMS query), you **MUST** instruct the `RetrieverAgent` to use `search_stored_documents_rag` to query these documents instead of doing web searches.
